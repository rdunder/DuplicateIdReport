 ...
 */
function findDuplicateIds_includingInputs() {
...
/**
 * Finds and reports all elements with duplicate IDs in the document
 * The function will exclude form input elements.
 * @returns {Object} Result object containing duplicate information
 */
function findDuplicateIds() {
    const elementsWithIds = document.querySelectorAll('[id]:not(input):not(select):not(textarea):not(button)');
    const allIds = Array.from(elementsWithIds).map(el => el.id);
    ...
    if (hasDuplicates) {
        uniqueDuplicateIds.forEach(dupId => {
            const elementsWithSameId = document.querySelectorAll(`#${CSS.escape(dupId)}:not(input):not(select):not(textarea):not(button)`);
            const instances = [];
